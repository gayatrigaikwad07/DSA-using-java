...
public class Cycle_Detection {
    ...
    public static boolean isCycleUtil(ArrayList<Edge>[] graph, int curr, boolean vis[], boolean stack[]) {
        ...
        for (Edge edge : graph[curr]) {
            ...
            if (!vis[neighbor]) {
                if (isCycleUtil(graph, neighbor, vis, stack)) {
                    return true;
                }
            } else if (stack[neighbor]) {
                // If the neighbor is already visited and is in the current stack, then there is a cycle.
                return true;
            }
        }

        // Remove the current node from the stack after processing.
        stack[curr] = false;
        return false;
    }
